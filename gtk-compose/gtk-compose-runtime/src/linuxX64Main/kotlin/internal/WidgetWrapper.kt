...
public open class WidgetWrapper(public open val widget: Widget) {
    ...
    public fun insert(index: Int, widgetWrapper: WidgetWrapper) {
        val length = children.size
        if (index < length) {
            val before = children[index]
            widgetWrapper.widget.insertBefore(widget, before)
            children.add(index, widgetWrapper.widget)
        } else {
            widgetWrapper.widget.insertAfter(widget, children.lastOrNull())
            children.add(widgetWrapper.widget)
        }
    }
